                ...
                <div class="row justify-content-center">
                    <div class="col-md-6">
                        ...
                        <div class="form-group">
                            <label for="" class="control-label">Org Member</label>
                            <select name="student_id" id="student_id" class="form-control select2 select2-sm" required>
                                <option></option> 
                                <?php 
                                    $stmt = $con->prepare("SELECT s.*, CONCAT(c.level, '-', c.section) as class, CONCAT(firstname, ' ', middlename, ' ', lastname) as name FROM students s INNER JOIN classes c ON c.id = s.class_id ORDER BY CONCAT(firstname, ' ', middlename, ' ', lastname) ASC");
                                    $stmt->execute();
                                    $students = $stmt->fetchAll(PDO::FETCH_ASSOC);

                                    foreach($students as $row):
                                ?>
                                    <option value="<?php echo $row['id'] ?>" data-class_id='<?php echo $row['class_id'] ?>' data-class='<?php echo $row['class'] ?>' <?php echo isset($student_id) && $student_id == $row['id'] ? "selected" : '' ?>>
                        ...
                        <div class="d-flex justify-content-center align-items-center">
                            <div class="form-group col-sm-4">
                                ...
                                <select name="" id="subject_id" class="form-control select2 select2-sm input-sm">
                                    <option></option> 
                                    <?php 
                                        $stmt = $con->prepare("SELECT * FROM subjects ORDER BY subject ASC");
                                        $stmt->execute();
                ...
                <div class="col-md-8 offset-md-2">
                    <table class="table table-bordered" id="mark-list">
                        ...
                        <tbody>
                            <?php if(isset($id)): ?>
                                <?php 
                                    $stmt = $con->prepare("SELECT r.*, s.subject_code, s.subject, s.id as sid FROM result_items r INNER JOIN subjects s ON s.id = r.subject_id WHERE result_id = :id ORDER BY s.subject_code ASC");
                                    $stmt->execute(['id' => $id]);
